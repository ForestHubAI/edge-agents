...
config** — and fills the bundle from it. Without a terminal, every _required_ value must be present
...
ModelBinding      { "location": "device",  "modelFile": string }
...
| `Agent` using a catalog model                    | nothing _blocking_ — but the matching `llmKeys` entry is needed at **build** time | **yes**                |
...
```json
{
  ...
  "models": {
    ...
    "vllm-remote": {
      ...
      "apiKey": "REPLACE_ME_MODEL_API_KEY"
    }
  },
  "webSearch": {
    "provider": "brave",
    "apiKey": "REPLACE_ME_WEB_SEARCH_API_KEY"
  },
