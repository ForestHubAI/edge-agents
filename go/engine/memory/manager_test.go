...
func TestManager_Reconcile_LocalWinsOverDeclared(t *testing.T) {
	// Warm volume: a local record exists, so edge-primary keeps it; the
	// declared seed never overwrites accumulated edits.
	dir := t.TempDir()
...
func TestManager_Reconcile_DeclaredMetadataIsAuthoritative(t *testing.T) {
