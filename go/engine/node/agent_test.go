...
func TestAgent_Outputs(t *testing.T) {
	t.Run("answer slot emitted only in emit mode", func(t *testing.T) {
		...
		assign := workflowapi.OutputBinding{
			Active: true,
		...
		assert.Equal(t, workflowapi.String, out["answer"])
	...
	t.Run("emit-mode declared outputs are included", func(t *testing.T) {
		emit := workflowapi.OutputBinding{Active: true, Mode: workflowapi.OutputBindingModeEmit}
		decls := []workflowapi.OutputDeclaration{
			{Name: "score", DataType: workflowapi.Int, Mode: workflowapi.OutputDeclarationModeEmit, Uid: pointer.Ptr("score-uid")},
			{Name: "tag", DataType: workflowapi.String, Mode: workflowapi.OutputDeclarationModeAssign,
				Target: &workflowapi.Reference{SrcId: engine.SrcDeclared, VarId: "x"}},
		}
		...
		assert.Equal(t, workflowapi.Int, out["score-uid"])
...
func TestAgent_BuildResponseFormat(t *testing.T) {
	...
	t.Run("includes declared outputs keyed by name", func(t *testing.T) {
		a := newAgentNode("a", emit, []workflowapi.OutputDeclaration{
			{Name: "score", DataType: workflowapi.Int, Mode: workflowapi.OutputDeclarationModeEmit, Uid: pointer.Ptr("u1")},
			{Name: "label", DataType: workflowapi.String, Mode: workflowapi.OutputDeclarationModeEmit, Uid: pointer.Ptr("u2")},
		})
		rf := a.buildResponseFormat()
		props, ok := rf.Schema["properties"].(map[string]any)
		require.True(t, ok)
...
func TestAgent_ApplyStructuredOutputs(t *testing.T) {
	...
	t.Run("writes answer plus emit-declared output", func(t *testing.T) {
		...
		ans, err := s.Resolve(workflowapi.Reference{SrcId: "a", VarId: agentAnswerOutID})
		require.NoError(t, err)
		assert.Equal(t, expr.StringVal("ok"), ans)

		sc, err := s.Resolve(workflowapi.Reference{SrcId: "a", VarId: "score-uid"})
		require.NoError(t, err)
	...
	t.Run("type-mismatched declared output errors", func(t *testing.T) {
		...
		require.NoError(t, err)
		decls := []workflowapi.OutputDeclaration{
			{Name: "score", DataType: workflowapi.Int, Mode: workflowapi.OutputDeclarationModeEmit, Uid: pointer.Ptr("u")},
		}
		a := newAgentNode("a", emit, decls)
	...
	t.Run("assign-mode declaration writes to target", func(t *testing.T) {
		s, err := engine.NewMainScope([]workflowapi.Variable{
			{Uid: "label", DataType: workflowapi.String},
		})
		require.NoError(t, err)
		decls := []workflowapi.OutputDeclaration{
			{
				Name: "tag", DataType: workflowapi.String,
				Mode:   workflowapi.OutputDeclarationModeAssign,
				Target: &workflowapi.Reference{SrcId: engine.SrcDeclared, VarId: "label"},
			},
		}
		a := newAgentNode("a", emit, decls)
		...
		require.NoError(t, err)
		v, err := s.Resolve(workflowapi.Reference{SrcId: engine.SrcDeclared, VarId: "label"})
		require.NoError(t, err)
	...
	t.Run("non-string answer falls back to empty string", func(t *testing.T) {
		...
		require.NoError(t, err)
		v, err := s.Resolve(workflowapi.Reference{SrcId: "a", VarId: agentAnswerOutID})
		require.NoError(t, err)
...
func TestAgent_Setup(t *testing.T) {
	...
	t.Run("declared outputs flip structuredResponse", func(t *testing.T) {
		decls := []workflowapi.OutputDeclaration{
			{Name: "n", DataType: workflowapi.Int, Mode: workflowapi.OutputDeclarationModeEmit, Uid: pointer.Ptr("u")},
		}
		a := newAgentNode("a", emit, decls)
		require.NoError(t, a.Setup(t.Context()))
...
func TestAgent_Next(t *testing.T) {
	...
	t.Run("single branch is taken regardless of choice", func(t *testing.T) {
		...
		a := newAgentNode("a", emit, nil)
		require.NoError(t, a.AddTransition(engine.PortCtrl, engine.Transition{
			TargetID: "only", EdgeType: workflowapi.AgentChoice,
		}))
	...
	t.Run("multi-branch routes via choice token", func(t *testing.T) {
		...
		a := newAgentNode("a", emit, nil)
		require.NoError(t, a.AddTransition(engine.PortCtrl, engine.Transition{
			TargetID: "first", EdgeType: workflowapi.AgentChoice, Description: pointer.Ptr("d1"),
		}))
		require.NoError(t, a.AddTransition(engine.PortCtrl, engine.Transition{
			TargetID: "second", EdgeType: workflowapi.AgentChoice, Description: pointer.Ptr("d2"),
		}))
	...
	t.Run("transition Apply error is wrapped", func(t *testing.T) {
		...
		a := newAgentNode("a", emit, nil)
		require.NoError(t, a.AddTransition(engine.PortCtrl, engine.Transition{
			TargetID: "first", EdgeType: workflowapi.AgentTask, Prompt: nil, // bad: AgentTask requires prompt
		}))
