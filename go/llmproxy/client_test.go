...
func TestNewClient_duplicateModel(t *testing.T) {
	// Two providers serving one model id are unroutable (Chat resolves by id
	// alone), so NewClient rejects the collision instead of silently overwriting.
	p1 := newTestProvider(t, "providerA")
	p2 := newTestProvider(t, "providerB")
	p1.EXPECT().AvailableModels().Return([]ModelInfo{{ID: "dup", Provider: "providerA"}})
	p2.EXPECT().AvailableModels().Return([]ModelInfo{{ID: "dup", Provider: "providerB"}})

	_, err := NewClient([]Provider{p1, p2})
	assert.Error(t, err)
