...
// DeploymentSpec The resolved set of components to run on one device for one deployment — a pure desired-state payload. Every decision (which components run, their images, device access, config file content) is computed once by the packaging step and frozen here, so a renderer renders it without re-deriving anything. Identity and lifecycle (which deployment this is, when it was produced, whether it is currently active) are NOT in the payload: they belong to the envelope that stores it — relational columns on the paid path, the deploy directory name/layout on the OSS path — so the store's own keys are never duplicated and each path owns history its own way. Payload + cached images + device-local env files are all a device needs to (re)start offline.
type DeploymentSpec struct {
	...
	// SchemaVersion Payload format version. Bumped when the spec shape changes incompatibly; travels with the payload so a reader knows how to parse components.
