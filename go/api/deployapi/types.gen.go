...
type DeployComponent struct {
	...
	// Pull Image pull policy, emitted as the renderer's pull_policy. Omit to default to "missing" (pull only if absent locally — correct for any registry image). Use "never" for an image built locally and present in no registry (the OSS engine); "always" re-pulls on every start.
