...
// Resolved relative to this module (not cwd) so the command works from anywhere.
// cli -> app -> ts -> repo root, then into contract/.
...
/**
 * `fh-builder check-schema <file.json>`
 *
...
export async function checkSchemaCommand(filePath?: string): Promise<void> {
  if (!filePath) {
    process.stderr.write("Usage: fh-builder check-schema <file.json>\n");
    process.exit(1);
  }
