import type { Expression, FunctionInfo, NodeDefinition, NodeInstance } from "../types/node";
import { NodeCategory, NodeRegistry } from "../types/node";
import type { EdgeInstance, EdgeType } from "../types/edge";
import { getEdgeDefinition, isControlFlow } from "../types/edge";
import { getArguments, getNodeAvailableOutput, getOutputBinding, getPorts } from "../types/node/NodeMethods";
import type { Edge } from "@xyflow/react";
import type { AvailableVariable } from "./variables";
import { computeAvailableVariables } from "../hooks/useAvailableVariables";
import { isExpression, resolveExpression } from "./expressions/types";
import { parseExpression } from "./expressions/parser";
import { isNodeUsedAsTool } from "./portUtils";
import { isParameterActive, resolveExpressionType, resolveChannelTypes } from "../types/parameter";
import type { ExpressionParam, ChannelSelectParam, OutputDeclaration } from "../types/parameter";
import { MAIN_CANVAS_ID } from "../store/canvasStore";
import type { ChannelInstance } from "../types/channel";
import { CHANNEL_DEFINITION } from "../types/channel";
import type { MemoryFileInstance } from "../types/memory";
...
import { FunctionCallNode } from "../types/node/FunctionNode";
import { exportSnapshot, type Snapshot, type CanvasData } from "./snapshots";
 ...
 * {@link Snapshot} (the store-independent serialized format used for
 * auto-save / versions / JSON export) — no Zustand stores, no React, no DOM.
 * Runnable in Node, a CLI, or a Claude Code skill.
 *
 * {@link validateAllCanvases} is the store-bound wrapper around this.
 */
export function validateWorkflow(snapshot: Snapshot): ValidationResult {
  const canvasData = snapshot.canvases ?? {};
  const allFunctions = deriveFunctionRegistry(canvasData);
  const channels = snapshot.channels ?? {};
  const memoryFiles = snapshot.memoryFiles ?? {};
...
/**
 * Store-bound full-project validation. Snapshots the live canvas/editor
 * stores via {@link exportSnapshot} and delegates to the headless
 * {@link validateWorkflow}. Behavior is identical to the previous
 * store-iterating implementation.
 *
 * Does NOT write to diagnosticsStore — that store only holds active-canvas
 * diagnostics populated by rendered nodes/edges.
 */
export function validateAllCanvases(): ValidationResult {
  return validateWorkflow(exportSnapshot());
}
