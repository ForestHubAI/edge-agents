...
// The hardware-channel families the engine has a driver for. UART and camera
  ...
  for (const channel of Object.values(workflow.channels)) {
    switch (channel.type) {
      ...
      case "UART":
        ...
        break;
      case "CAMERA":
        hardwareChannels.push({ id: channel.id, label: channel.label, family: "camera", addressable: false });
        break;
      case "MQTT":
