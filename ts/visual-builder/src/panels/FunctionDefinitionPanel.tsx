...
export const FunctionDefinitionPanel = ({
  ...
  if (!functionInfo) {
    return (
      <div className="p-4">
        <p className="text-sm text-muted-foreground">{t("builder.noFunctionInfo")}</p>
      </div>
    );
  }
    ...
    <div className="p-4">
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="function-name">{t("builder.functionName")}</Label>
          <Input
            id="function-name"
            value={functionInfo.name}
            onChange={handleNameChange}
            readOnly={readOnly}
            placeholder={t("builder.functionNamePlaceholder")}
            className="font-mono"
          />
        </div>
          ...
          <>
            ...
            <div className="space-y-3">
              <Label className="text-sm font-medium">
                {t("builder.returnValueAssignments")}
              </Label>
              <p className="text-xs text-muted-foreground">
                {t("builder.returnValueAssignmentsDesc")}
              </p>
                ...
                {functionInfo.returns.map((output) => (
                  <div key={output.uid} className="space-y-1">
                    ...
                    <ExpressionInput
                      ...
                      placeholder={`${t("builder.expressionFor")} ${output.name}...`}
                    />
                  </div>
                ))}
              </div>
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("builder.deleteFunctionTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("builder.deleteFunctionDesc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
