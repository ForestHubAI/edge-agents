...
export const BuilderSidebar = ({
  ...
  const staticTabs = useMemo(() => [
    { id: "nodes" as const, icon: Blocks, label: t("builder.nodeLibrary") },
    { id: "variables" as const, icon: Variable, label: t("builder.variables") },
    { id: "channels" as const, icon: Cpu, label: t("builder.channels") },
    { id: "memory" as const, icon: Database, label: t("builder.memoryFiles", "Memory") },
    { id: "diagnostics" as const, icon: TriangleAlert, label: t("builder.diagnostics") },
  ], [t]);

  const functionTab = useMemo(
    () => ({ id: "function" as const, icon: Braces, label: t("builder.functionDefinition") }),
    [t],
  );

  const debugTabs = useMemo(() => [
    { id: "debug-context" as const, icon: Bug, label: t("builder.debug.context") },
  ], [t]);
