  ...
  if (list.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        ...
        <p className="text-sm text-muted-foreground">{t("builder.noMemoryFiles", "No memory files yet")}</p>
        <p className="text-xs text-muted-foreground/70 mt-1">
          {t("builder.noMemoryFilesHint", "Declare durable storage your agents can read or write")}
        </p>
        {!readOnly && (
          <Button variant="outline" size="sm" className="mt-3" onClick={handleAdd}>
            <Plus className="w-3.5 h-3.5 mr-1" />
            {t("builder.addMemoryFile", "Add Memory File")}
          </Button>
        )}
      </div>
    );
