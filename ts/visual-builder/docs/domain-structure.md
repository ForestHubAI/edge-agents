...
The domain layer lives in `src/types/` and defines the data models for nodes, edges, and their serialization. It sits between the API schema (`src/api/openapi.yaml` → `src/api/generated.ts`) and the visual-builder UI.
...
| **Domain types** | `src/types/node/`, `src/types/edge/`           | Runtime models used by the visual builder and consumed by the validator        |
