...
The visual builder lets users wire data between nodes. Underneath, three fundamental concepts make this work:
...
### Canvas variable system (`src/visual-builder/utils/variables.ts`)
...
| `src/visual-builder/utils/variables.ts`             | Canvas variable types, key helpers, `refToLookupKey()`                  |
| `src/visual-builder/store/canvasStore.ts`           | `CanvasState.variables`, `computeVariablesFromNodes()`                  |
| `src/visual-builder/hooks/useAvailableVariables.ts` | Computes the current canvas's variables (self-contained per canvas)     |
| `src/visual-builder/utils/variables.ts`                        | Also exports `ensureUid()`, `paramKey()` — uid management utilities     |
| `src/visual-builder/utils/expressions/types.ts`      | `resolveExpression()`, `ResolvedExpr` — expression ↔ display conversion |
| `src/visual-builder/inputs/ExpressionInput.tsx`     | Builds `Reference` objects when user selects variables                  |
| `src/visual-builder/inputs/ParameterEditor.tsx`     | Variable-reference dropdowns, uses `refToLookupKey()`                   |
| `src/visual-builder/hooks/useImportExport.ts`       | Bidirectional conversion between API format and canvas stores           |
