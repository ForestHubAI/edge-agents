        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex-1 min-w-0">
            <div className="group flex items-center gap-1.5 rounded-md border border-transparent px-1.5 -mx-1.5 hover:border-input focus-within:border-input transition-colors">
              <input
                type="text"
                title={t("memoryFileName", "Memory file name")}
            ...
            <p className="text-sm text-muted-foreground">
              {t("memoryFileDescription", "Agent-scoped durable storage")}
            </p>
            {isEmptyName && (
              <p className="text-xs text-destructive mt-1">
                {t("memoryFileNameRequired", "Name is required")}
              </p>
            )}
            {isDuplicateName && (
              <p className="text-xs text-destructive mt-1">
                {t("memoryFileNameDuplicate", "Name must be unique per agent")}
              </p>
            )}
          </div>
        ...
        {!readOnly && (
          <>
            ...
            <Button variant="destructive" className="w-full" onClick={() => deleteMemoryFile(memoryFile.uid)}>
              ...
              {t("deleteMemoryFile", "Delete memory file")}
            </Button>
          </>
        )}
      </div>
