...
| File | Role |
|------|------|
| `src/workflow-builder/hooks/useGraph.ts` | Public mutation API — readOnly gate + auto-checkpoint wrapper |
| `src/workflow-builder/utils/graphOperations.ts` | Pure mutation functions — no React, no gating, no checkpointing |
| `src/workflow-builder/store/canvasStore.ts` | Raw store — `setNodes`/`setEdges` used by ReactFlow directly |
| `src/workflow-builder/CanvasArea.tsx` | ReactFlow integration — `onNodesChange`/`onEdgesChange` write to raw store |
| `src/workflow-builder/CanvasLayout.tsx` | Orchestrates mutations via `useGraph` |
