...
| File | Role |
|------|------|
| `src/workflow-builder/store/editorStore.ts` | Orchestration selection state (`selectedNodeIds`, `selectedEdgeIds`) |
| `src/workflow-builder/store/canvasStore.ts` | Raw store — ReactFlow writes `node.selected` here |
| `src/workflow-builder/graph/BaseNode.tsx` | Reads `selected` prop for glow |
| `src/workflow-builder/graph/CustomEdge.tsx` | Reads `selected` prop for glow |
| `src/workflow-builder/CanvasLayout.tsx` | Bridges ReactFlow events → editorStore; reads editorStore for panels/keyboard |
| `src/workflow-builder/CanvasArea.tsx` | Passes ReactFlow selection props (`onSelectionChange`, `onPaneClick`) |
