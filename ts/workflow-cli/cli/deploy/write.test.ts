...
describe("writeOutput", () => {
  ...
  // Windows ignores POSIX file modes (every file reports 0o666), so the bit-level
  // assertion is meaningful only on the deploy target. Skip it off-POSIX.
  it.skipIf(process.platform === "win32")("0o600s every secret-bearing file (config, spec, env)", async () => {
    const base = await tmp();
    const out = path.join(base, "bundle");
    await writeOutput(specOf(), cfgOf(out), reqOf());
    expect(await mode(path.join(out, ".env"))).toBe(0o600);
    expect(await mode(path.join(out, "engine-config.json"))).toBe(0o600);
    expect(await mode(path.join(out, "deployment-spec.json"))).toBe(0o600);
